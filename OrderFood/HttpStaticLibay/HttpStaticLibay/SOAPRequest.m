                           ...
                           wsParameters:(NSMutableArray *) wsParas
{
    ...
    NSString * soapMsgBody1 = [[[NSString alloc] initWithFormat:
                               ...
                               "<soap:Body>\n"
                               "<%@ xmlns=\"%@\">\n", wsName, xmlNS] autorelease];
    NSString * soapMsgBody2 = [[[NSString alloc] initWithFormat:
                               @"</%@>\n"
                               "</soap:Body>\n"
                               "</soap:Envelope>", wsName] autorelease];
    
    //2、生成SOAP调用参数
    NSString * soapParas = [[[NSString alloc] init] autorelease];
    soapParas = @"";
...
+ (NSString *)checkResponseError:(NSString *) theResponse
{
    ...
    if (![theResponse hasPrefix:[Constant sharedConstant].G_WEBSERVICE_ERROR]) {
        ...
        NSMutableString *sTemp = [[[NSMutableString alloc] initWithString:theResponse] autorelease];
        //获取错误前缀的范围
...
{
    //创建请求
    ASIHTTPRequest * theRequest = [[self class] getASISOAP11Request:WebURL
                                             webServiceFile:wsFile
                                               xmlNameSpace:xmlNS
                                             webServiceName:wsName
                                               wsParameters:wsParas];
...
{
    //返回字符串使用 NSString *responseString = [request responseString ];
    //对于二进制数据，使用： NSData *responseData = [request responseData];
    
    NSData *responseData = [request responseData];
    
    if (delegate && [delegate respondsToSelector:@selector(SOAPRequest:didFinishRequest:)])
    {
        [delegate SOAPRequest:self didFinishRequest:responseData];
    }
}
...
- ( void )requestFailed:( ASIHTTPRequest *)request
{
    ...
    if (delegate && [delegate respondsToSelector:@selector(SOAPRequest:responseErrorOccurred:)])
    {
        [delegate SOAPRequest:self responseErrorOccurred:error];
    }
}
